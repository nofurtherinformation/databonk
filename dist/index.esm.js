...
function inferArrayDataType(values) {
    if (values.length === 0)
        return 'float64';
    let maxType = 'int8';
...
class Column {
    constructor(name, values, dataType) {
        this.name = name;
    ...
    slice(start, end) {
        ...
        return new Column(this.name, slicedValues, this.dataType);
    }
    filter(predicate) {
        ...
        return new Column(this.name, filteredValues, this.dataType);
    }
    map(fn, newDataType) {
        const mappedValues = [];
        for (let i = 0; i < this.length; i++) {
...
class Joiner {
    ...
    static validateJoinKeys(left, right, keys) {
        keys.forEach(key => {
    ...
    static buildJoinedDataFrame(left, right, matches, joinKeys, suffixes) {
        const resultColumns = {};
        matches.length;
...
DataFrame.prototype.join = function (other, on, how = 'inner', suffixes = ['_x', '_y']) {
    return Joiner.join(this, other, on, how, suffixes);
};

class Reshaper {
    static pivot(df, index, columns, values, aggFunc = 'mean') {
    ...
    static wide_to_long(df, stubnames, i, j = 'id', sep = '_') {
        const idColumns = i;
        const wideColumns = [];
        ...
        const suffixes = wideColumns[0].map(col => col.substring(col.lastIndexOf(sep) + 1));
        df.length * suffixes.length;
        const resultColumns = {};
        idColumns.forEach(colName => {
            const values = [];
            const originalColumn = df.column(colName);
    ...
    static computeAggregateValue(column, fn) {
        switch (fn) {
            case 'sum':
                return column.sum();
            case 'mean':
...
DataFrame.prototype.pivot = function (index, columns, values, aggFunc = 'mean') {
    return Reshaper.pivot(this, index, columns, values, aggFunc);
};
DataFrame.prototype.melt = function (id_vars, value_vars, var_name = 'variable', value_name = 'value') {
...
class ColumnDeriver {
    static withColumn(df, name, expression, dataType) {
        const values = [];
        for (let i = 0; i < df.length; i++) {
    ...
    static withColumns(df, expressions) {
        let result = df;
    ...
    static transform(df, columnName, transformer, newColumnName, dataType) {
    ...
    static assign(df, assignments) {
        return this.withColumns(df, assignments);
    }
    static fillna(df, columnName, value) {
        return this.transform(df, columnName, (val) => val === null ? value : val, columnName);
    }
    static dropna(df, columnNames) {
        const columnsToCheck = columnNames || df.columnNames;
        return df.filter(row => {
            return columnsToCheck.every(colName => row[colName] !== null);
        });
    }
    static replace(df, columnName, oldValue, newValue) {
        return this.transform(df, columnName, (val) => val === oldValue ? newValue : val, columnName);
    }
    static clip(df, columnName, lower, upper) {
        const column = df.column(columnName);
        if (column.dataType === 'string' || column.dataType === 'boolean') {
            throw new Error('Cannot clip non-numeric column');
        }
        return this.transform(df, columnName, (val) => {
            if (val === null)
                return null;
            let clipped = val;
            if (lower !== undefined && clipped < lower)
                clipped = lower;
            if (upper !== undefined && clipped > upper)
                clipped = upper;
            return clipped;
        }, columnName, 'float64');
    }
    static round(df, columnName, decimals = 0) {
        const column = df.column(columnName);
        if (column.dataType === 'string' || column.dataType === 'boolean') {
    ...
    static abs(df, columnName) {
        const column = df.column(columnName);
        if (column.dataType === 'string' || column.dataType === 'boolean') {
            throw new Error('Cannot take absolute value of non-numeric column');
        }
        return this.transform(df, columnName, (val) => val === null ? null : Math.abs(val), columnName, 'float64');
    }
    static sqrt(df, columnName) {
        const column = df.column(columnName);
        if (column.dataType === 'string' || column.dataType === 'boolean') {
            throw new Error('Cannot take square root of non-numeric column');
        }
        return this.transform(df, columnName, (val) => val === null ? null : Math.sqrt(val), columnName, 'float64');
    }
    static log(df, columnName, base) {
        const column = df.column(columnName);
        if (column.dataType === 'string' || column.dataType === 'boolean') {
            throw new Error('Cannot take logarithm of non-numeric column');
        }
        const logFn = base ? (x) => Math.log(x) / Math.log(base) : Math.log;
        return this.transform(df, columnName, (val) => val === null || val <= 0 ? null : logFn(val), columnName, 'float64');
    }
    static normalize(df, columnName) {
        const column = df.column(columnName);
        if (column.dataType === 'string' || column.dataType === 'boolean') {
            throw new Error('Cannot normalize non-numeric column');
        }
        const mean = column.mean();
        const values = column.toArray().filter(v => v !== null);
        const variance = values.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / (values.length - 1);
        const std = Math.sqrt(variance);
        return this.transform(df, columnName, (val) => val === null ? null : (val - mean) / std, columnName, 'float64');
    }
    static rank(df, columnName, ascending = true) {
        const column = df.column(columnName);
...
DataFrame.prototype.withColumn = function (name, expression, dataType) {
    return ColumnDeriver.withColumn(this, name, expression, dataType);
};
DataFrame.prototype.withColumns = function (expressions) {
    return ColumnDeriver.withColumns(this, expressions);
};
DataFrame.prototype.assign = function (assignments) {
    return ColumnDeriver.assign(this, assignments);
};
DataFrame.prototype.fillna = function (columnName, value) {
    return ColumnDeriver.fillna(this, columnName, value);
};
DataFrame.prototype.dropna = function (columnNames) {
    return ColumnDeriver.dropna(this, columnNames);
};
DataFrame.prototype.replace = function (columnName, oldValue, newValue) {
    return ColumnDeriver.replace(this, columnName, oldValue, newValue);
};
DataFrame.prototype.clip = function (columnName, lower, upper) {
    return ColumnDeriver.clip(this, columnName, lower, upper);
};
DataFrame.prototype.round = function (columnName, decimals = 0) {
    return ColumnDeriver.round(this, columnName, decimals);
};
DataFrame.prototype.abs = function (columnName) {
    return ColumnDeriver.abs(this, columnName);
};
DataFrame.prototype.sqrt = function (columnName) {
    return ColumnDeriver.sqrt(this, columnName);
};
DataFrame.prototype.log = function (columnName, base) {
    return ColumnDeriver.log(this, columnName, base);
};
DataFrame.prototype.normalize = function (columnName) {
    return ColumnDeriver.normalize(this, columnName);
};
DataFrame.prototype.rank = function (columnName, ascending = true) {
...
class Aggregator {
    ...
    static computeStd(column) {
        return Math.sqrt(this.computeVar(column));
    }
}
DataFrame.prototype.agg = function (spec) {
    return Aggregator.agg(this, spec);
};
DataFrame.prototype.describe = function () {
    return Aggregator.describe(this);
};
DataFrame.prototype.sum = function (columns) {
...
DataFrame.prototype.mean = function (columns) {
    const cols = columns || this.columnNames.filter(name => {
        const col = this.column(name);
        return col.dataType !== 'string' && col.dataType !== 'boolean';
    });
...
DataFrame.prototype.count = function (columns) {
...
DataFrame.prototype.min = function (columns) {
    const cols = columns || this.columnNames.filter(name => {
        const col = this.column(name);
        return col.dataType !== 'string' && col.dataType !== 'boolean';
    });
...
DataFrame.prototype.max = function (columns) {
    const cols = columns || this.columnNames.filter(name => {
        const col = this.column(name);
        return col.dataType !== 'string' && col.dataType !== 'boolean';
    });
...
class SchemaValidator {
    ...
    static validate(df, schema, options = {}) {
    ...
    static validateAndTransform(df, schema) {
...
class SchemaBuilders {
    ...
    static date() {
        return dateType().or(stringType().datetime()).transform(val => val instanceof Date ? val : new Date(val));
    }
    static enum(values) {
...
class DataGenerator {
    ...
    static generateLargeDataset(populationRows, options = {}) {
        ...
        if (includeNulls) {
            ...
            for (let i = 0; i < nullCount; i++) {
                ...
                if (Math.random() < 0.33)
                    populationData[randomIndex].population1 = 0;
                if (Math.random() < 0.33)
                    populationData[randomIndex].population2 = 0;
                if (Math.random() < 0.33)
                    populationData[randomIndex].population3 = 0;
            }
        }
...
export { Aggregator, BitSet, Column, ColumnDeriver, CommonSchemas, CsvReader, CsvWriter, DataFrame, DataGenerator, GroupBy, Joiner, MemoryProfiler, PerformanceTimer, Reshaper, SchemaBuilders, SchemaValidator };
